...
FROM
	SYS.DUAL
	LEFT JOIN (SELECT ';' || CHR(10) || SYS.DBMS_METADATA.GET_DEPENDENT_DDL(OBJECT_TYPE => 'INDEX', BASE_OBJECT_NAME => :NAME, BASE_OBJECT_SCHEMA => :SCHEMA) SCRIPT_INDEX FROM ALL_INDEXES WHERE TABLE_NAME = :NAME AND TABLE_OWNER = :SCHEMA AND ROWNUM = 1) ON 1 = 1
	LEFT JOIN (SELECT ';' || CHR(10) || SYS.DBMS_METADATA.GET_DEPENDENT_DDL(OBJECT_TYPE => 'COMMENT', BASE_OBJECT_NAME => :NAME, BASE_OBJECT_SCHEMA => :SCHEMA) SCRIPT_COMMENT FROM (SELECT NULL FROM ALL_TAB_COMMENTS WHERE TABLE_NAME = :NAME AND OWNER = :SCHEMA UNION ALL SELECT NULL FROM ALL_COL_COMMENTS WHERE TABLE_NAME = :NAME AND OWNER = :SCHEMA) WHERE ROWNUM = 1) ON 1 = 1
	LEFT JOIN (SELECT ';' || CHR(10) || SYS.DBMS_METADATA.GET_DEPENDENT_DDL(OBJECT_TYPE => 'TRIGGER', BASE_OBJECT_NAME => :NAME, BASE_OBJECT_SCHEMA => :SCHEMA) SCRIPT_TRIGGER FROM ALL_TRIGGERS WHERE TABLE_NAME = :NAME AND OWNER = :SCHEMA AND ROWNUM = 1) ON 1 = 1";
...
@"SELECT
	...
	CASE WHEN SQL_ID IS NOT NULL AND BITAND(OWNERID, 65535) = 65532 THEN
		(SELECT SQL_TEXT FROM V$SQL WHERE V$SESSION.SQL_ID = SQL_ID AND V$SESSION.SQL_CHILD_NUMBER = CHILD_NUMBER AND ROWNUM = 1)
	END CURRENT_COMMAND_TEXT,
	CASE WHEN PREV_SQL_ID IS NOT NULL AND BITAND(OWNERID, 65535) = 65532 THEN
		(SELECT SQL_TEXT FROM V$SQL WHERE V$SESSION.PREV_SQL_ID = SQL_ID AND V$SESSION.PREV_CHILD_NUMBER = CHILD_NUMBER AND ROWNUM = 1)
