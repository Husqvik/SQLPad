...
END;";
		...
		public const string SelectActiveSessionHistoryCommandText = "SELECT SAMPLE_TIME, SESSION_STATE, SESSION_ID, SESSION_SERIAL#, SQL_PLAN_LINE_ID, WAIT_CLASS, WAIT_TIME, TIME_WAITED, TM_DELTA_TIME, TM_DELTA_CPU_TIME, TM_DELTA_DB_TIME, DELTA_TIME, DELTA_READ_IO_REQUESTS, DELTA_WRITE_IO_REQUESTS, DELTA_READ_IO_BYTES, DELTA_WRITE_IO_BYTES, DELTA_INTERCONNECT_IO_BYTES, DELTA_READ_MEM_BYTES, PGA_ALLOCATED, TEMP_SPACE_ALLOCATED FROM V$ACTIVE_SESSION_HISTORY WHERE (SESSION_ID = :SID OR QC_SESSION_ID = :SID) AND SQL_EXEC_ID = :SQL_EXEC_ID AND SQL_EXEC_START = :SQL_EXEC_START AND SAMPLE_TIME > :SAMPLE_TIME ORDER BY SESSION_ID, SAMPLE_ID";
...
@"SELECT
	...
	V$SESSION.SERIAL#, AUDSID, RAWTOHEX(PADDR) PADDR, V$SESSION.USERNAME,
	TADDR, LOCKWAIT, INITCAP(STATUS) STATUS, INITCAP(SERVER) SERVER, SCHEMANAME, OSUSER, PROCESS, MACHINE, NULLIF(PORT, 0) PORT, V$SESSION.PROGRAM, INITCAP(TYPE) TYPE,
...
FROM
	V$SESSION
	LEFT JOIN V$PROCESS ON V$SESSION.PADDR = V$PROCESS.ADDR";
